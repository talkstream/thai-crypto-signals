...
ชุดทดสอบสร้าง **D1, KV และ queue จริงในเครื่อง** ไม่มีการปลอมส่วนที่เก็บข้อมูล มีเพียงคำขอออกไปยัง
Bitkub คำขอเดียวเท่านั้นที่ถูกแทนด้วยของจำลอง นี่คือความหมายของ "การทดสอบกับโครงสร้างจริง"
และเป็นเหตุผลที่ชุดทดสอบจับ bug จริงได้
...
เล็กน้อยถือเป็นปกติ เพราะบางตลาดมีฝั่งเดียวหรือไม่อยู่ในรายการจึงถูกข้าม)
...
**ค่าใช้จ่าย.** cron, D1 และ KV อยู่ในโควต้า **ฟรี** ของ Cloudflare ได้สบายสำหรับงานระดับนี้
**Queues ต้องใช้แพ็กเกจ Workers Paid** — และ queue ถูกใช้โดยโครง signals ที่ปิดอยู่เท่านั้น
ดังนั้นหากต้องการอยู่ในโควต้าฟรี ให้ลบบล็อก `queues` ออกจาก `wrangler.jsonc` ก่อน deploy
...
3. **(ปานกลาง)** เขียนเทสต์สำหรับกรณีล้มเหลวแบบใหม่ เช่น ticker คืน HTTP 500 *คำใบ้:* ขอบเขตเดียวที่ถูก
   แทนด้วยของจำลองคือ `globalThis.fetch` ดูตัวอย่างใน `test/integration/collect.test.ts`
4. **(ขั้นสูง)** เปิดใช้งานโครง **signals** ที่ปิดอยู่ และส่งข้อความ Telegram หนึ่งครั้งเมื่อราคาขยับ
   เกิน X% *คำใบ้:* `src/signals/` ต่อสายไว้แล้ว แต่การส่งถูกปิดด้วยแฟล็ก `SIGNALS_ENABLED`
   (เป็น `var` ใน `wrangler.jsonc`; secret ใส่ด้วย `pnpm exec wrangler secret put …`) ทำอย่างสุภาพ
   และจำกัดอัตราการส่ง
