...
ชุดทดสอบสร้าง **D1 และ KV จริงในเครื่อง** ส่วนที่เก็บข้อมูลไม่ได้ถูกปลอม ขอบเขตที่ถูกแทนด้วยของจำลองมีสองจุด คือคำขอออกไปยัง
Bitkub (ใช้คำตอบที่บันทึกไว้) และตัว producer ของ queue เพราะ test runtime ในเครื่องไม่มี API ฝั่ง producer นี่คือความหมายของ "การทดสอบกับโครงสร้างจริง"
...
**ค่าใช้จ่าย.** โปรเจกต์นี้ทั้งหมดอยู่ในโควต้า **ฟรี** ของ Cloudflare ได้ ทั้ง cron, D1, KV และ —
ตั้งแต่เดือนกุมภาพันธ์ 2026 — Queues (10,000 operations ต่อวัน) ก็รวมอยู่ในนั้นแล้ว งานระดับนี้ไม่ต้องใช้แพ็กเกจ Workers Paid
